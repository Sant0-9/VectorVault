...
TEST_F(DistanceTest, L2NaiveVsSIMD) {
    const float tolerance =
        5e-3f;  // Increased for accumulated floating point errors in high dimensions
    ...
    for (size_t i = 0; i < test_dims.size(); ++i) {
        ...
        EXPECT_LT(diff, tolerance)
            << "Dimension: " << dim << ", Naive: " << naive_result << ", SIMD: " << simd_result;
    }
}
...
TEST_F(DistanceTest, CosineRangeCheck) {
    // Cosine distance should be in range [0, 2] (1 - cosine_similarity where similarity is in [-1,
    // 1])
    for (size_t i = 0; i < test_dims.size(); ++i) {
        int dim = test_dims[i];
        const auto& a = test_vectors_a[i];
