...
bool MMapFile::open_read(const std::string& path) {
    ...
    data_ = static_cast<uint8_t*>(MapViewOfFile(mapping_handle_, FILE_MAP_READ, 0, 0, 0));
...
bool MMapFile::open_write(const std::string& path, size_t size) {
    ...
    file_handle_ = CreateFileA(path.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
    ...
    mapping_handle_ =
        CreateFileMappingA(file_handle_, nullptr, PAGE_READWRITE, static_cast<DWORD>(size >> 32),
    ...
    data_ = static_cast<uint8_t*>(MapViewOfFile(mapping_handle_, FILE_MAP_WRITE, 0, 0, 0));
...
uint32_t BinaryReader::read_uint32() {
    if (pos_ + sizeof(uint32_t) > size_) {
        throw std::runtime_error("BinaryReader: not enough data");
    }
    uint32_t value;
